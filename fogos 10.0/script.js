...
class Firework {
    ...
    explode() {
        this.exploded = true;
        for (let i = 0; i < 100; i++) {  // Aumenta o número de partículas
...
class Spark {
    constructor(x, y, color) {
        ...
        this.color = color;
        this.speedX = (Math.random() - 0.5) * 6;  // Aumenta a velocidade e direção
        this.speedY = (Math.random() - 0.5) * 6;
        this.gravity = 0.05;
        this.alpha = 1;
        this.life = 3;  // Define o tempo de vida em 3 segundos
        this.friction = 0.98;
    }
    ...
    update() {
        ...
        this.y += this.speedY;
        this.speedY += this.gravity;
        this.alpha -= 1 / (this.life * 60);  // Diminui a opacidade ao longo do tempo
    }
...
function animate() {
    ...
    // Desenha a água e suas ondas
    water.updateWaves();
    water.draw();

    // Gera novos fogos de artifício aleatoriamente
    if (Math.random() < 0.05) {
    ...
    fireworks.forEach((firework, index) => {
        ...
        // Remove fogos de artifício que não têm mais partículas
